...
import '../styles/global.css'; // ✅ 여기서만 로드
...
export default function LoginPage() {
  ...
  const onChange = (e) => {
    ...
    if (name === 'email') setFieldErr((fe) => ({ ...fe, email: validateEmail(value) ? '' : '올바른 이메일 주소를 입력하세요.' }));
    if (name === 'password') setFieldErr((fe) => ({ ...fe, password: value ? '' : '비밀번호를 입력하세요.' }));
  };
  ...
  return (
    <div className="auth-scope">{/* ✅ 스코프 시작 */}
      <div className="auth-card" role="main" aria-labelledby="login-title">
        ...
        <form onSubmit={handleSubmit} className="auth-form" noValidate>
          <label className="field">
            ...
            <input name="email" type="email" placeholder="example@domain.com" value={form.email}
          ...
          <label className="field">
            ...
            <div className="pw-field">
              <input name="password" type={showPw ? 'text' : 'password'} placeholder="비밀번호"
          ...
          <Link to="/register" className="linklike" aria-label="회원가입으로 이동">회원가입</Link>
        </form>
      </div>
    </div>
  );
